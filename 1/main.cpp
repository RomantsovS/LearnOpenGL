...
int main() {
    ...
    glShaderSource(fragmentShader, 1, &fragmentShaderSource[0], NULL);
    ...
    glShaderSource(fragmentShader, 1, &fragmentShaderSource[1], NULL);
    glCompileShader(fragmentShader);
    // check for shader compile errors
    glGetShaderiv(fragmentShader, GL_COMPILE_STATUS, &success);
    if (!success) {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glBindVertexArray(
            VAOs[0]);  // seeing as we only have a single VAO there's no need to bind it
