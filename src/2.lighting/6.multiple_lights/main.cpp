...
int main() {
    ...
    // ------------------------------------
    Shader lightingShader("src/2.lighting/6.multiple_lights/6.multiple_lights.vs",
                          "src/2.lighting/6.multiple_lights/6.multiple_lights.fs");
    Shader lightCubeShader("src/2.lighting/6.multiple_lights/6.light_cube.vs",
                           "src/2.lighting/6.multiple_lights/6.light_cube.fs");
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        lightingShader.setVec3("dirLight.diffuse", glm::vec3(0.5));
        lightingShader.setVec3("dirLight.specular", glm::vec3(0.8));

        std::vector<glm::vec3> lightColors(pointLightPositions.size());

        for (size_t i = 0; i < pointLightPositions.size(); ++i) {
            lightColors[i].x = sin(glfwGetTime() * (i + 1) * 0.2);
            lightColors[i].y = cos(glfwGetTime() * (i + 1));
            lightColors[i].z = sin(glfwGetTime() * (i + 1) * 0.5);

            glm::vec3 diffuseColor = lightColors[i] * glm::vec3(0.5f);
            glm::vec3 ambientColor = diffuseColor * glm::vec3(0.2f);

            pointLightPositions[i].x = std::sin((float)glfwGetTime() * (i + 1)) * (i + 1);
            pointLightPositions[i].y = std::cos((float)glfwGetTime() / (i + 1)) * i;
            pointLightPositions[i].z = std::cos((float)glfwGetTime() * (i + 1)) * (i + 1);
